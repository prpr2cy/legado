...
class EpubFile(var book: Book) {
     ...
     * 从OPF文件中获取字符集
     */
    private fun getCharsetFromOpf(): Charset {
        return try {
            val opfResource = epubBook?.resources?.getByHref("OEBPS/content.opf")
                ?: epubBook?.resources?.getByHref("content.opf")
                ?: return StandardCharsets.UTF_8

            val opfContent = String(opfResource.data, StandardCharsets.UTF_8)
            ...
            for (item in items) {
                ...
                if (mediaType.contains("application/xhtml+xml") || mediaType.contains("text/html")) {
                    // 检查media-type是否包含字符集信息
                    if (mediaType.contains("charset=")) {
                        val charset = mediaType.substringAfter("charset=").trim()
                        return Charset.forName(charset)
                    }
                }
            }

            // 如果OPF文件中没有字符集信息，返回默认字符集
     ...
     * 从XHTML文件中获取字符集
     */
    private fun getCharsetFromXhtml(res: Resource): Charset {
        return try {
            val xhtmlContent = String(res.data, StandardCharsets.UTF_8)
            val doc = Jsoup.parse(xhtmlContent)

            // 查找<meta charset="...">标签
            val metaCharset = doc.select("meta[charset]").first()
            if (metaCharset != null) {
                val charset = metaCharset.attr("charset").trim()
                return Charset.forName(charset)
            }

            // 查找<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">标签
            val metaContentType = doc.select("meta[http-equiv=Content-Type]").first()
            if (metaContentType != null) {
                val content = metaContentType.attr("content")
                if (content.contains("charset=")) {
                    val charset = content.substringAfter("charset=").trim()
                    return Charset.forName(charset)
                }
            }
    ...
    /**
     * 检测字符集（优先从OPF文件获取，其次从XHTML文件获取）
     */
    private fun detectCharset(res: Resource): Charset {
        return try {
            // 1. 优先从OPF文件获取字符集
            val opfCharset = getCharsetFromOpf()
            if (opfCharset != StandardCharsets.UTF_8) {
                return opfCharset
            }

            // 2. 从XHTML文件获取字符集
            val xhtmlCharset = getCharsetFromXhtml(res)
    ...
    private fun getBody(res: Resource, startFragmentId: String?, endFragmentId: String?): Element {
        ...
        bodyElement.select("object").forEach {
            it.tagName("img", Parser.NamespaceHtml)
            it.attr("src", it.attr("data"))
        }
