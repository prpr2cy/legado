...
class ContentTextView(context: Context, attrs: AttributeSet?) : View(context, attrs) {
    ...
    private fun drawImage(
        ...
        kotlin.runCatching {
            // 分页时只绘制要显示的区域
            if (column.totalPages > 1) {
                val srcRect = Rect(0, column.cropStartY, bitmap.width, column.cropEndY)
                canvas.drawBitmap(bitmap, srcRect, rectF, imagePaint)
                return@runCatching
            }
            canvas.drawBitmap(bitmap, null, rectF, imagePaint)
        }.onFailure {
            AppLog.put("drawImage error", it)
        }
    }
