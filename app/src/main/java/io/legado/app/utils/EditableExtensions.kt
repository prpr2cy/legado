...
private class EditableQueue(val editableRef: WeakReference<Editable>) {
...
private val queueMap = mutableMapOf<WeakReference<Editable>, EditableQueue>()
...
* 处理指定队列（使用burst优化）
*/
private fun processQueue(queue: EditableQueue) {
   if (!queue.isValid()) {
